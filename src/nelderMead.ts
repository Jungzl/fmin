...
interface NelderMeadParameters {
  ...
  psi?: number
  sigma?: number
  history?: Array<{ x: number[]; fx: number; simplex: SimplexPoint[] }>
}
...
export function nelderMead(
  ...
  simplex[0] = x0.slice() as SimplexPoint
  simplex[0].fx = f(x0)
  simplex[0].id = 0
  for (let i = 0; i < N; ++i) {
    const point = x0.slice() as SimplexPoint
  ...
  function updateSimplex(value: SimplexPoint): void {
  ...
  const sortOrder = (a: SimplexPoint, b: SimplexPoint) => a.fx - b.fx

  const centroid = x0.slice()
  const reflected = x0.slice() as SimplexPoint
  const contracted = x0.slice() as SimplexPoint
  const expanded = x0.slice() as SimplexPoint
  ...
  for (let iteration = 0; iteration < maxIterations; ++iteration) {
    ...
    if (parameters.history) {
      ...
      const sortedSimplex = simplex.map(x => {
        const state = x.slice() as SimplexPoint
        state.fx = x.fx
        state.id = x.id
        return state
      })
